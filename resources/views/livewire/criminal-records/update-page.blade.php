<div>
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Criminal Record</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-zinc-400">For {{ $record->person->full_name }}</p>
        </div>
        <x-ui.button href="{{ route('people.view', $record->person_id) }}" variant="secondary"
        ...
        <x-form.card title="Offense Details">
            ...
            <x-slot:footer>
                <div class="flex items-center justify-end gap-3">
                    <x-ui.button href="{{ route('people.view', $record->person_id) }}" variant="secondary">Cancel</x-ui.button>
                    <x-ui.button type="submit" wire:loading.attr="disabled" wire:loading.class="opacity-75">Save Changes</x-ui.button>
                </div>
            </x-slot:footer>
        </x-form.card>
    </form>
