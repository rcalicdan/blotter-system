<x-form.section title="Involved Parties" description="Add people involved in this incident and their roles.">
    <div class="space-y-4">
        @foreach ($parties as $index => $party)
            <div wire:key="party-{{ $index }}" class="flex items-start gap-3 p-4 bg-gray-50 dark:bg-zinc-800/50 rounded-xl border border-gray-100 dark:border-zinc-700">
                <div class="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {{-- Person Search --}}
                    <div>
                        <x-form.label :required="true">Person</x-form.label>
                        <livewire:form.searchable-dropdown
                            wire:model="parties.{{ $index }}.person_id"
                            :model="\App\Models\Person::class"
                            :searchFields="['first_name', 'last_name']"
                            displayField="full_name"
                            placeholder="Search person..."
                            :key="'party-person-'.$index"
                        />
